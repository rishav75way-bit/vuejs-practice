...
import { createEmptyExperience } from '@/utils/types'
...
const formData = ref(createEmptyExperience())

const openAddModal = () => {
  formData.value = createEmptyExperience()
...
<template>
  <div class="flex flex-col gap-4">
    <div class="flex items-center justify-between">
      <h3 class="text-lg font-semibold text-gray-900 m-0">Work Experience</h3>
    ...
    <div v-else class="flex flex-col gap-3">
      ...
      >
        <div class="flex-1">
          <h4 class="text-[15px] font-semibold text-gray-900 m-0 mb-1">{{ item.title || 'Untitled' }}</h4>
          <p class="text-sm text-gray-600 m-0 mb-1">{{ item.company }}</p>
    ...
    <BaseModal :show="showModal" :title="editingItem ? 'Edit Experience' : 'Add Experience'" @close="closeModal">
      <div class="flex flex-col gap-4">
        <BaseInput
        ...
        <BaseInput
          ...
          required
          @update:model-value="updateFormField('company', $event)"
        />
        <BaseInput
